...
class FastBlurHelper {
    ...
    fun doBlur(sentBitmap: Bitmap, radius: Int, canReuseInBitmap: Boolean): Bitmap?{
        ...
        val hm = h - 1
        val wh = w * h
        val div = attr.radius + attr.radius + 1
...

        {
            ...
            while (y < h) {
                ...
                while (x < w) {
                    ...
                    gsum -= goutsum
                    bsum -= boutsum
                    stackstart = stackpointer - attr.radius + div
                    sir = stack[stackstart % div]
                    routsum -= sir[0]
                    goutsum -= sir[1]
                    boutsum -= sir[2]
                    if (y == 0) {
                        vmin[x] = Math.min(x + attr.radius + 1, wm)
                    }
                    p = pix[yw + vmin[x]]
                    sir[0] = p and 0xff0000 shr 16
                    sir[1] = p and 0x00ff00 shr 8
